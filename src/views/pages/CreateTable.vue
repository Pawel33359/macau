            ...
            <form @submit.prevent="handleSubmit">
                ...
                <div class="row">
                    ...
                    <div class="row" v-if="ifranking == false">
                        <div class="col-md-2 col-sm-4"><button class="btn btn-primary" @click="ifranking = true">Yes</button></div>
                        <div class="col-md-2 col-sm-4"><button class="btn btn-success" disabled>No</button></div>
                    </div>
                    <!--Ranking buttons if it is a ranking game-->
                    <div class="row" v-else>
                        <div class="col-md-2 col-sm-4"><button class="btn btn-success" disabled>Yes</button></div>
                        <div class="col-md-2 col-sm-4"><button class="btn btn-primary" @click="ifranking = false">No</button></div>
                    </div>
                    <!--Password-->
                    ...
                    <div class="row" v-if="ifpassword == false">
                        <div class="col-md-2 col-sm-4"><button class="btn btn-primary" @click="ifpassword = true">Yes</button></div>
                        <div class="col-md-2 col-sm-4"><button class="btn btn-success" disabled>No</button></div>
                        <div class="col-12 mt-2"><input type="password" placeholder="Password" v-model="password" disabled></div>
                    </div>
                    <!--Password buttons for password option-->
                    <div class="row" v-else>
                        <div class="col-md-2 col-sm-4"><button class="btn btn-success" disabled>Yes</button></div>
                        <div class="col-md-2 col-sm-4"><button class="btn btn-primary" @click="handleClick">No</button></div>
                        <div class="col-12 mt-2"><input type="password" placeholder="Password" v-model="password"></div>
                    </div>
                </div>
...
import getTables from '@/composables/getTables'
import getWatchedtables from "@/composables/getWatchedtables"
//other
...
export default {
    ...
    setup(){
                ...
                if(tables.value.length > 0){
                    ...
                    if(password.value.length < 4 && ifpassword.value == true){
                        ...
                        const table = {
                            ...
                            messages: [],
                            inprogress: false,
                            users: [{userid: user.value.uid, name: user.value.displayName, ready: false}]
                        }
        ...
        return {error,
        password,name,tables,
        ifranking,ifpassword,isPending,
