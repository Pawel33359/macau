<template>
    <div v-if="profile" class="singlefriend">
        ...
        <div class="options">
            <div @click="accept" class="opt check"><font-awesome-icon icon="check" class="sign"/></div>
            <div @click="decline" class="opt times"><font-awesome-icon icon="times" class="sign"/></div>
        </div>
    </div>
</template>
...
export default {
    ...
    setup(props){
        ...
        const { error, updateDoc, deleteDoc } = useDocument("friendlist", props.doc.id);
