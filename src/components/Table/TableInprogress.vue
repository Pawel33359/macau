<template>
<!--choosing functions-->
    <div v-if="(documents.function.type =='ace' || documents.function.type =='joker' || 
    documents.function.type =='demandnot' || documents.function.type =='demandjoker' || 
    documents.function.type =='demandjokerjack') 
    && documents.currentturn.turn==placement[0]" class="w-100 h-100">
        <div v-if="documents.function.type =='ace'">
            <Ace :table="table" :documents="documents" :user="user" :player="player"/>
        </div>
        <div v-if="documents.function.type =='joker' || documents.function.type =='demandjoker' || documents.function.type =='demandjokerjack'" class="h-100 w-100">
            <Joker :table="table" :documents="documents" :nextuserid="nextuserid" :previoususerid="previoususerid" :user="user" :player="player"/>
        </div>
        <div v-if="documents.function.type =='demandnot'" class="h-100 w-100">
            <Demand :table="table" :documents="documents" :user="user" :player="player"/>
        </div>
    </div>
    <!--players and drawpile and throwpile-->
    <div v-else class="w-100 h-100">
        <div class="demandscreen">
            <div class="demand" v-if="documents.function.type=='demandyes' 
            || documents.function.type=='demandjoker' 
            || documents.function.type=='demandjokerjack'"
            >
                <div class="playerinfo">Demanded</div>
                <div class="playerinfo" v-if="documents.function.value==12" >card: Q</div>
                <div class="playerinfo" v-else>card: {{documents.function.value}}</div>
                <div class="playerinfo">Turns left: {{documents.function.amount}}</div>
            </div>
        </div>
        <div class="draw w-25 h-25 row">
            <div class="col-xs-12 col-sm-6 mb-2 p-0">
                <img @click="draw(user.uid)" src="@/assets/cards/b1.png" width="80" class="drawpile">
            </div>
        ...
        <div v-if="documents.playercount.number == 2">
            <div v-for="player in documents.users" :key="player.information.turn" >
                <div v-if="player.information.turn == placement[0]" class="playercards w-50">
                    <!--playerinformation-->
                    <div class="mainplayer" 
                    v-if="player.cards"
                    :class="{ 
                        currentturn: player.information.turn == documents.currentturn.turn, 
                    ...
                    <div v-for="card in player.cards" :key="card.id" class="card">
                        <img @click="throwcard(user.uid,card,nextuserid,previoususerid)" :src="card.path" width="80" class="owned">
                    </div>
                    <!--makao button-->
                    <div v-if="documents.makao.user == player.information.id">
                        <button @click="makao(player.information.id)" class="btn btn-warning makao">MAKAO</button>
                    </div>
                </div>

                <div v-if="player.information.turn == placement[1]" class="playerfrontcards w-50">
                     <!--playerinformation-->
                    ...
                    <!--makao button-->
                    <div v-if="documents.makao.user == player.information.id">
                        <button @click="stopmakao(player.information.id)" class="btn btn-danger stop-makao-front">STOP MAKAO</button>
                    </div>
                </div>
            </div>
        </div>
        <div v-else>
            <div v-for="player in documents.users" :key="player.information.turn" >
                    ...
                    <div v-if="player.information.turn == placement[1]" class="playerleftcards h-50">
                        <!--playerinformation-->
                        ...
                        <!--makao button-->
                        <div v-if="documents.makao.user == player.information.id">
                            <button @click="stopmakao(player.information.id)" class="btn btn-danger stop-makao-left">STOP MAKAO</button>
                        </div>
                    </div>

                <div v-if="player.information.turn == placement[2]" class="playerfrontcards w-50">
                    <!--playerinformation-->
                    <div class="frontplayer" 
                     v-if="player.cards"
                    :class="{ 
                ...
                <div v-if="player.information.turn == placement[3]" class="playerrightcards h-50">
                    <!--playerinformation-->
                    ...
                    <!--makao button-->
                    <div v-if="documents.makao.user == player.information.id">
                        <button @click="stopmakao(player.information.id)" class="btn btn-danger stop-makao-right">STOP MAKAO</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    setup(props){
        ...
        //get user playing
        const { documents: player } = getDatabase("tables/" + props.table.id + "/users/" + props.user.uid);
        
        //get all users
        const { documents } = getDatabase("tables/" + props.table.id);


        const {draw,throwcard,makao,stopmakao} = game(props.table,player.value,documents.value,props.profiles,props.rankings)
        ...
        for(var checkuser of users){
            const { documents: nuser } = getDatabase("tables/" + props.table.id + "/users/" + checkuser);
        ...
        return {placement,draw,throwcard,documents,nextuserid,previoususerid,makao,stopmakao,
        player
        }
    }
}
