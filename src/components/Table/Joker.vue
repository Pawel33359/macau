<template>
 ...
 <!--demand cards-->
 <div class="cards" v-if="documents.function.type == 'demandjoker' || documents.function.type == 'demandjokerjack'">
     Hello
    <div class="alldemand">
        <div class="demand">
            <div v-for="card in demand" :key="card.suit" >
                <img :src="card.path" width="100" @click="chosen = card" 
...
export default {
    ...
    setup(props){
        ...
        if(props.documents.function.type == "demandjoker" || props.documents.function.type == "demandjokerjack"){
            for(var i=0; i<4; i++){
                var path = require('@/assets/cards/'+suits[i]+props.documents.function.value+".png")
                var objecthand = {suit: suits[i], value: props.documents.function.value, path: path}
        ...
        const handleAccept = ()=>{
            ...
            const {functiondemand,turndatabase,demanddecrease,
            function23,functionstop,functionking,functionsuit,
            ifmakao
            } = game(props.table,props.player,props.documents,[],[])

            //changemiddlecard
            const { addDat: addmiddlecard } = useDatabase("tables/" + props.table.id + "/middlecard/");
                ...
                addDat({amount: 0, type: ""})
                 //check if it was function card
                        if(chosen.value.value == 2 || chosen.value.value == 3){
                            ...
                            functionking(chosen.value,props.nextuserid,props.previoususerid,props.user.uid);
